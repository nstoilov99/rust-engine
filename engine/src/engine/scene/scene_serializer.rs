...
use super::scene_format::{
    ...
    RenderModeData, RigidBodyTypeData, SceneFile, SpawnShapeData, UpdateModuleData,
};
...
fn serialize_entity(world: &World, entity: Entity) -> Option<EntityData> {
    ...
    if world.get::<&StaticCollision>(entity).is_ok() {
        components.push(ComponentData::StaticCollision);
    }
    ...
    if let Ok(effect) = world.get::<&ParticleEffect>(entity) {
        let modules_data: Vec<UpdateModuleData> = effect
            .update_modules
            .iter()
            .map(|m| match m {
                ...
                UpdateModule::ColorOverLife { start, end } => UpdateModuleData::ColorOverLife {
                    start: *start,
                    end: *end,
                },
                UpdateModule::SizeOverLife { start, end } => UpdateModuleData::SizeOverLife {
                    start: *start,
                    end: *end,
                },
            })
        ...
        components.push(ComponentData::ParticleEffect {
            ...
            spawn_shape: match effect.spawn_shape {
                ...
                SpawnShape::Box { half_extents } => SpawnShapeData::Box { half_extents },
            },
    ...
    for component_data in &entity_data.components {
        match component_data {
            ...
            ComponentData::ParticleEffect {
                ...
                let modules: Vec<UpdateModule> = update_modules
                    .iter()
                    .map(|m| match m {
                ...
                builder.add(ParticleEffect {
                    ...
                    spawn_shape: match spawn_shape {
                        ...
                        SpawnShapeData::Sphere { radius } => SpawnShape::Sphere { radius: *radius },
                        SpawnShapeData::Cone { angle_rad, radius } => SpawnShape::Cone {
                            angle_rad: *angle_rad,
                            radius: *radius,
                        },
                        SpawnShapeData::Box { half_extents } => SpawnShape::Box {
