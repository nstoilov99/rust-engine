    ...
    pub fn show(&mut self, ctx: &egui::Context) -> Vec<EditorAction> {
        ...
        egui::Window::new(dialog.title.clone())
            ...
            .show(ctx, |ui| {
                ...
                ui.add_space(12.0);
                ui.horizontal(|ui| match dialog.buttons {
                    DialogButtons::Ok => {
                        if ui.button("OK").clicked() {
                            if let Some(action) = dialog.actions.primary.clone() {
                                actions.push(action);
                            }
                            close = true;
                        }
                    ...
                    DialogButtons::OkCancel => {
                        ...
                        if ui.button("Cancel").clicked() {
                            if let Some(action) = dialog.actions.cancel.clone() {
                                actions.push(action);
                            }
                            close = true;
                        }
                    ...
                    DialogButtons::SaveDiscardCancel => {
                        ...
                        if ui.button("Discard").clicked() {
                            if let Some(action) = dialog.actions.secondary.clone() {
                                actions.push(action);
                            }
                            close = true;
                        }
