...
pub(crate) struct RotationState {
    ...
    current_delta: f64,
    /// Cached sign (1.0 or -1.0) computed at drag start.
    /// This ensures the angle calculation doesn't flip mid-drag due to view vector changes.
    angle_sign: f64,
    /// Cached gizmo screen position at drag start.
    /// This ensures the reference point for angle calculation is consistent throughout the drag.
...
impl SubGizmoKind for Rotation {
        ...
        Self: Sized,
    {
        ...
        let angle = if subgizmo.direction == GizmoDirection::View {
            ...
            // Z-up fork: Negate forward to fix arc picking - ensures visible arc (facing camera) is pickable
    ...
    fn pick(subgizmo: &mut RotationSubGizmo, ray: Ray) -> Option<f64> {
        ...
        // Cache the angle sign FIRST, before computing any rotation angles.
        // This prevents 180° rotation jumps when the view direction crosses the rotation plane.
        let dot = subgizmo.config.view_forward()
            .dot(gizmo_normal(&subgizmo.config, subgizmo.direction));
        subgizmo.state.angle_sign = if dot < 0.0 { -1.0 } else { 1.0 };

        // Cache the gizmo screen position at drag start.
        // This ensures the reference point for angle calculation is consistent throughout the drag.
        subgizmo.state.start_screen_pos = world_to_screen(
            subgizmo.config.viewport,
            subgizmo.config.view_projection,
            subgizmo.config.translation
        );

        // NOW compute start_rotation_angle using the cached sign and screen position.
        // This ensures consistency with update() which also uses the cached values.
        // Using pick_result.rotation_angle would be wrong because it was computed
        // with a fresh sign that might differ from the cached one.
        let start_rotation_angle = rotation_angle_with_sign(
            subgizmo,
            ray.screen_pos,
            Some(subgizmo.state.angle_sign),
            subgizmo.state.start_screen_pos
        ).unwrap_or(0.0);

        subgizmo.state.start_rotation_angle = start_rotation_angle;
        subgizmo.state.last_rotation_angle = start_rotation_angle;
    ...
    fn update(subgizmo: &mut RotationSubGizmo, ray: Ray) -> Option<GizmoResult> {
        ...
        // Use cached sign and screen position from pick() to ensure consistent angle calculation throughout drag
        let mut rotation_angle = rotation_angle_with_sign(
            subgizmo,
            ray.screen_pos,
            Some(subgizmo.state.angle_sign),
            subgizmo.state.start_screen_pos
    ...
    fn draw(subgizmo: &RotationSubGizmo) -> GizmoDrawData {
        ...
        draw_data += shape_builder
                .polyline(
                    &[
                        DVec3::new(0.0, start_angle.cos() * radius, start_angle.sin() * radius),
                        DVec3::new(0.0, 0.0, 0.0),
                        DVec3::new(0.0, end_angle.cos() * radius, end_angle.sin() * radius),
                    ],
                    stroke,
                )
                .into();
            ...
            if config.snapping {
                ...
                for i in 0..((TAU / config.snap_angle as f64) as usize + 1) {
                    ...
                    // YZ plane: X=0, Y=cos, Z=sin
                    let pos = DVec3::new(0.0, angle.cos(), angle.sin());
                    draw_data += shape_builder
                        .line_segment(
                            pos * radius * 1.1,
                            pos * radius * 1.2,
...
fn rotation_matrix(subgizmo: &SubGizmoConfig<Rotation>) -> DMat4 {
    if subgizmo.direction == GizmoDirection::View {
        ...
        // Circle is drawn in YZ plane (X=0). For screen-aligned circle:
        // - Column 0 (local X) = -camera forward (circle normal points toward camera)
        // - Column 1 (local Y) = camera right → circle's Y extends right on screen
        // - Column 2 (local Z) = camera up → circle's Z extends up on screen
        let rotation = DQuat::from_mat3(&DMat3::from_cols(-forward, right, up));

        return DMat4::from_rotation_translation(rotation, subgizmo.config.translation);
    }

    // First rotate towards the gizmo normal
    // Z-up fork: Use X as the reference axis since circles are in YZ plane (normal along X)
    let local_normal = gizmo_local_normal(&subgizmo.config, subgizmo.direction);
    let rotation = rotation_align(DVec3::X, local_normal);
...
/// Calculate the rotation angle from cursor position.
/// If `cached_sign` is provided, it uses that sign instead of computing fresh.
/// If `cached_screen_pos` is provided, it uses that screen position instead of computing fresh.
/// This is used during drag to ensure consistent angle calculation throughout the drag.
fn rotation_angle_with_sign(
    subgizmo: &SubGizmoConfig<Rotation>,
    cursor_pos: Pos2,
    cached_sign: Option<f64>,
    cached_screen_pos: Option<Pos2>
) -> Option<f64> {
    let viewport = subgizmo.config.viewport;
    // Use cached screen position if provided, otherwise compute fresh.
    // Caching ensures the reference point is consistent throughout the drag,
    // avoiding issues where the gizmo's screen position might shift slightly between frames.
    let gizmo_pos = cached_screen_pos.or_else(||
        world_to_screen(viewport, subgizmo.config.view_projection, subgizmo.config.translation)
    )?;
    let delta = DVec2::new(
    ...
    // Use cached sign if provided (during drag), otherwise compute fresh (during pick)
    let sign = cached_sign.unwrap_or_else(|| {
        let dot = subgizmo
            .config
            .view_forward()
            .dot(gizmo_normal(&subgizmo.config, subgizmo.direction));
...
/// Calculate the rotation angle from cursor position, computing the sign and screen position fresh.
/// Used in pick_preview() before the drag starts.
fn rotation_angle(subgizmo: &SubGizmoConfig<Rotation>, cursor_pos: Pos2) -> Option<f64> {
    rotation_angle_with_sign(subgizmo, cursor_pos, None, None)
}

fn tangent(subgizmo: &SubGizmoConfig<Rotation>) -> DVec3 {
    // Z-up fork: Tangent vectors for Z-up coordinate system
    // The tangent defines the reference direction for rotating the arc toward the camera.
    // It should be perpendicular to the rotation axis (normal).
    //
    // In Z-up: X=forward, Y=right, Z=up
    let mut tangent = match subgizmo.direction {
        GizmoDirection::X => -DVec3::Z,  // X rotation: tangent perpendicular to X
        GizmoDirection::Y => -DVec3::Z,  // Y rotation: tangent perpendicular to Y
        GizmoDirection::Z => DVec3::X,   // Z rotation: tangent perpendicular to Z, pointing forward
