...
impl ShapeBuilder {
    ...
    fn arc_points(&self, radius: f64, start_angle: f64, end_angle: f64) -> Vec<Pos2> {
        ...
        for step in (0..step_count).map(|i| step_size * i as f64) {
            // Z-up: Generate circle in YZ plane (X=0) so it faces forward
            let y = f64::cos(start_angle + step) * radius;
            let z = f64::sin(start_angle + step) * radius;

            points.push(DVec3::new(0.0, y, z));
        }
    ...
    pub(crate) fn sector(
        ...
        for _ in 0..step_count {
            // Z-up: Generate sector in YZ plane (X=0) so it faces forward
            let y = cos_angle * radius;
