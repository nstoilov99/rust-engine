...
pub struct GizmoConfig {
    ...
    /// Optional: Camera forward direction in world space.
    /// If set, overrides the forward vector extracted from view_matrix.
    pub view_forward_override: Option<mint::Vector3<f64>>,
    /// Optional: Camera up direction in world space.
    /// If set, overrides the up vector extracted from view_matrix.
    pub view_up_override: Option<mint::Vector3<f64>>,
    /// Optional: Camera right direction in world space.
    /// If set, overrides the right vector extracted from view_matrix.
    pub view_right_override: Option<mint::Vector3<f64>>,
}
...
impl Default for GizmoConfig {
    fn default() -> Self {
        Self {
            ...
            view_forward_override: None,
            view_up_override: None,
            view_right_override: None,
        }
    }
}
...
impl Default for GizmoVisuals {
    fn default() -> Self {
        // Z-up fork: Standard RGB axis colors for Z-up coordinate system
        // X = Red (forward), Y = Green (right), Z = Blue (up)
        Self {
            x_color: Color32::from_rgb(255, 0, 0),     // Red - X axis (forward)
            y_color: Color32::from_rgb(0, 255, 0),    // Green - Y axis (right)
            z_color: Color32::from_rgb(0, 0, 255),    // Blue - Z axis (up)
