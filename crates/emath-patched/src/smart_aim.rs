...
pub fn best_in_range_f64(min: f64, max: f64) -> f64 {
    ...
    for i in 0..NUM_DECIMALS {
        if min_str[i] == max_str[i] {
            ...
            // Handle floating-point precision edge cases where rounding
            // causes min_str[i] >= max_str[i]. Fall back to returning min.
            if deciding_digit_min >= deciding_digit_max {
                return min;
            }
