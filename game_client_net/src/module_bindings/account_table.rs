...
/// Access to the `character_id` unique index on the table `account`,
/// which allows point queries on the field of the same name
/// via the [`AccountCharacterIdUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.account().character_id().find(...)`.
pub struct AccountCharacterIdUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<Account, u64>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> AccountTableHandle<'ctx> {
    /// Get a handle on the `character_id` unique index on the table `account`.
    pub fn character_id(&self) -> AccountCharacterIdUnique<'ctx> {
        AccountCharacterIdUnique {
...
impl<'ctx> AccountCharacterIdUnique<'ctx> {
