...
/// Access to the `owner_identity` unique index on the table `player`,
/// which allows point queries on the field of the same name
/// via the [`PlayerOwnerIdentityUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.player().owner_identity().find(...)`.
pub struct PlayerOwnerIdentityUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<Player, __sdk::Identity>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> PlayerTableHandle<'ctx> {
    /// Get a handle on the `owner_identity` unique index on the table `player`.
    pub fn owner_identity(&self) -> PlayerOwnerIdentityUnique<'ctx> {
        PlayerOwnerIdentityUnique {
            imp: self
                .imp
...
impl<'ctx> PlayerOwnerIdentityUnique<'ctx> {
